package jwt

import (
	"errors"
	"time"

	"github.com/golang-jwt/jwt"
)

// JWT signing Key
type JWT struct {
	SigningKey []byte
}

var (
	ErrTokenExpired     = errors.New("token expired")
	ErrTokenNotValidYet = errors.New("token is not active yet")
	ErrTokenMalformed   = errors.New("that's not even a token")
	ErrTokenInvalid     = errors.New("couldn't handle this token")
)

// CustomClaims Structured version of Claims Section, as referenced at https://tools.ietf.org/html/rfc7519#section-4.1 See examples for how to use this with your own claim types
type CustomClaims struct {
	Id int64
	jwt.StandardClaims
}

func NewJWT() *JWT {
	return &JWT{
		[]byte("Dousheng_Backend_Signingkey"),
	}
}

// CreateToken create a new token
func (j *JWT) CreateToken(claims CustomClaims) (string, error) {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims)
	return token.SignedString(j.SigningKey)
}

// ParseToken parse the token.
func (j *JWT) ParseToken(tokenString string) (*CustomClaims, error) {
	token, err := jwt.ParseWithClaims(tokenString, &CustomClaims{}, func(token *jwt.Token) (interface{}, error) {
		return j.SigningKey, nil
	})
	if err != nil {
		if ve, ok := err.(*jwt.ValidationError); ok {
			if ve.Errors&jwt.ValidationErrorMalformed != 0 {
				return nil, ErrTokenMalformed
			} else if ve.Errors&jwt.ValidationErrorExpired != 0 {
				return nil, ErrTokenExpired
			} else if ve.Errors&jwt.ValidationErrorNotValidYet != 0 {
				return nil, ErrTokenNotValidYet
			} else {
				return nil, ErrTokenInvalid
			}

		}
	}
	if claims, ok := token.Claims.(*CustomClaims); ok && token.Valid {
		return claims, nil
	}
	return nil, ErrTokenInvalid
}

func (j *JWT) GenTokenWithId(id int64) (string, error) {
	userJwt := NewJWT()
	token, err := userJwt.CreateToken(CustomClaims{
		Id: id,
		StandardClaims: jwt.StandardClaims{
			//ExpiresAt: time.Now().Add(time.Second * 5).Unix(),
			ExpiresAt: time.Now().Add(time.Hour * 24).Unix(),
			Issuer:    "dousheng",
		},
	})
	return token, err
}

func (j *JWT) GetIdFromToken(token string) (int64, error) {
	idJwt := NewJWT()
	claims, err := idJwt.ParseToken(token)
	var message string
	if err != nil {
		message = "token解析错误，无法获取用户ID"
		return -1, errors.New(message)
	}
	return claims.Id, nil
}
